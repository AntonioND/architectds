...
#include "gl2d/ruins/ruins_texture_png.h"
#include "gl2d/ruins/ruins.h"

const uint32_t screen_width = 256;
const uint32_t screen_height = 192;

glImage ruins[ruins_NUM_IMAGES];
...
int main(int argc, char **argv)
{
    ...
    // "ruins_texcoords", where "ruins" is a string defined by you when packing
    // the images.
    //
    ...
    int ruins_texture_id =
        glLoadSpriteSet(ruins,    // Pointer to glImage array
                        ruins_NUM_IMAGES, // Number of images
                        ruins_texcoords,  // Array of UV coordinates
                        ...
                        ruins_texture_pngPal,     // Pointer to texture palette data
                        ruins_texture_pngBitmap); // Pointer to texture data
    ...
    while (1)
    {
        ...
        glBegin2D();
            ...
            glSprite(0, 4, GL_FLIP_NONE, &ruins[ruins_1_png]);
            glSprite(64, 4, GL_FLIP_NONE, &ruins[ruins_2_png]);
            glSprite(128, 4, GL_FLIP_NONE, &ruins[ruins_3_png]);
            glSprite(192, 4, GL_FLIP_NONE, &ruins[ruins_4_png]);
            glSprite(0, 100, GL_FLIP_NONE, &ruins[ruins_5_png]);
            glSprite(80, 100, GL_FLIP_NONE, &ruins[ruins_6_png]);
            glSprite(180, 100, GL_FLIP_NONE, &ruins[ruins_7_png]);
